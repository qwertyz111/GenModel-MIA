# -*- coding: utf-8 -*-
"""
Step4 (ViT-cosine): Eval MIA by ViT encoder + two centroids, cosine distance only.

Unified score semantics:
    score(x) = d(x, c_noisy) - d(x, c_synth)
where d(x,c) = 1 - cos(x,c).
So score >= 0 means "closer to synth centroid" (member-like).
...
- roc_csv: ROC points
- roc_png: ROC plot
- thr_scan_csv: threshold vs ACC (+ repeated AUC, TPR@FPR=1%)

Offline:
- local ViT via --vit_dir with TRANSFORMERS_OFFLINE/HF_HUB_OFFLINE set
"""
...
class ViTEncoder(torch.nn.Module):
    def __init__(self, vit_dir: str, device: torch.device):
        super().__init__()
        self.processor = AutoImageProcessor.from_pretrained(vit_dir, local_files_only=True)
        self.model = ViTModel.from_pretrained(vit_dir, local_files_only=True).to(device)
        self.model.eval()
...
def parse_args():
    ap = argparse.ArgumentParser("Step4: Eval MIA (ViT + cosine only)")
    ap.add_argument("--centroids_pt", type=str, required=True)
    ap.add_argument("--vit_dir", type=str, required=True)
    ...
    # cosine-diff score range theoretically in [-2,2]
    ap.add_argument("--scan_min", type=float, default=-2.0)
    ap.add_argument("--scan_max", type=float, default=2.0)
    ap.add_argument("--scan_num", type=int, default=401)
...
def main():
    ...
    enc = ViTEncoder(args.vit_dir, device=device)

    C = torch.load(args.centroids_pt, map_location="cpu")
    meta = C.get("meta", None)
    if isinstance(meta, dict):
        enc_meta = meta.get("encoder", {})
        if isinstance(enc_meta, dict) and enc_meta.get("type", None) not in (None, "vit"):
            print(f"[WARN] centroids encoder meta is not vit: {enc_meta}")
    ...
    print("[INFO] score(x) = d(x,c_noisy) - d(x,c_synth) = cos(x,c_synth) - cos(x,c_noisy)")
    ...
    def score_from_emb(E_cpu: torch.Tensor) -> torch.Tensor:
        E = E_cpu.to(device)
        d_noisy = dist_cosine(E, c_noisy_d)
        d_synth = dist_cosine(E, c_synth_d)
        s = d_noisy - d_synth
        return s.detach().cpu()
    ...
    with open(args.scores_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for tag, s in zip(mem_tags, s_mem.numpy()):
            w.writerow(["member", tag, float(s), int(s >= args.threshold), "cosine"])
        for tag, s in zip(non_tags, s_non.numpy()):
            w.writerow(["nonmember", tag, float(s), int(s >= args.threshold), "cosine"])
    ...
    plt.title("ROC (ViT Encoder-Centroid MIA) - cosine")
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(args.roc_png, dpi=150)
    plt.close()

    print("\n=== MIA (ViT Encoder-Centroid) ===")
    ...
    with open(thr_scan_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for thr, acc_thr in scan_rows:
            w.writerow([thr, acc_thr, float(auc), float(tpr1), "cosine"])

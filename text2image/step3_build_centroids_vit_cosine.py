# -*- coding: utf-8 -*-
"""
Step3 (ViT-cosine): Build centroids from synth_dir and noisy_dir using ViT CLS embeddings.
...
Offline:
- local ViT via --vit_dir and TRANSFORMERS_OFFLINE/HF_HUB_OFFLINE
"""
...
def parse_args():
    ap = argparse.ArgumentParser("Step3: Build Centroids (ViT + cosine only)")
...
def main():
    ...
    meta = dict(
        encoder=dict(
            type="vit",
            repo_or_dir=os.path.abspath(args.vit_dir),
            pooling="cls",
            normalize=True,
        ),
        roles=dict(

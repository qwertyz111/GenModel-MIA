...
# -*- coding: utf-8 -*-
"""
Unified MIA-style experiment + DP-SGD-style defense on embeddings.

Defense: DP-SGD style sanitization on encoder embeddings:
  - per-sample L2 clipping
  - Gaussian noise addition

This is the closest DP-SGD-equivalent defense for an embedding+Gaussian MIA pipeline
(without training a neural classifier).

Outputs:
  - ROC figs
  - CSV summary (36 rows by default) with extra dp_clip/dp_noise columns
"""
...
# --------------------- DP-SGD style defense ----------------------

def dpsgd_defense_embeddings(
    embs: np.ndarray,
    clip_norm: float,
    noise_multiplier: float,
) -> np.ndarray:
    """
    DP-SGD style defense on per-sample embeddings:
      1) per-sample L2 clipping to clip_norm
      2) add Gaussian noise N(0, (noise_multiplier * clip_norm)^2 I)

    Note:
      - This is not a full DP guarantee w/ accountant (no epsilon reported),
        but it is the standard DP-SGD-equivalent sanitization for representation-based pipelines.
    """
    if clip_norm <= 0.0 or noise_multiplier <= 0.0:
        return embs
...
def run_for_dataset_model(
    ...
    # 3.5) DP-SGD-style defense on embeddings (recommended: apply once globally)
    if dp_clip > 0 and dp_noise > 0:
        print(f"Applying DP-SGD defense on embeddings: clip={dp_clip}, noise_mult={dp_noise}")
        emb_real_all = dpsgd_defense_embeddings(emb_real_all, dp_clip, dp_noise)
        emb_syn_all  = dpsgd_defense_embeddings(emb_syn_all,  dp_clip, dp_noise)
    ...
    for N in train_sizes:
        ...
        for noise_label, noise_flag in [("clean", False), ("noise", True)]:
            # "noise" here is your original eval-time perturbation (kept as-is)
            if noise_flag:
            ...
            fig_name = f"{dataset}_{model_short}_{N}_{noise_label}_dpclip{dp_clip}_dpnoise{dp_noise}.png"
...
def main():
    ...
    # DP-SGD defense params
    ap.add_argument("--dp_clip", type=float, default=0.0,
                    help="DP-SGD defense: embedding clip norm (0=disable)")
    ap.add_argument("--dp_noise", type=float, default=0.0,
                    help="DP-SGD defense: noise multiplier (0=disable)")

    ap.add_argument("--csv_path", type=str, default="results_summary.csv",
    ...
    # -------- Summary table --------
    print("\n=== SUMMARY (36 configs by default) ===")
    header = ["dataset", "model", "train_size", "noise", "dp_clip", "dp_noise", "accuracy", "auc", "tpr_at_1fpr"]
    print("{:<8} {:<7} {:<10} {:<7} {:<8} {:<9} {:<10} {:<8} {:<12}".format(*header))

    csv_path = args.csv_path
    with open(csv_path, "w", encoding="utf-8") as f:
        f.write(",".join(header) + "\n")
        for r in results:
            print("{:<8} {:<7} {:<10} {:<7} {:<8.3f} {:<9.3f} {:<10.4f} {:<8.4f} {:<12.4f}".format(
                r["dataset"], r["model"], r["train_size"], r["noise"],
                r["dp_clip"], r["dp_noise"],
                r["accuracy"], r["auc"], r["tpr_at_1fpr"]
            ))
            f.write("{dataset},{model},{train_size},{noise},{dp_clip:.6f},{dp_noise:.6f},{accuracy:.6f},{auc:.6f},{tpr_at_1fpr:.6f}\n".format(**r))

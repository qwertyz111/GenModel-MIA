...
# ---------------------- Distance-based MIA scores ------------------------

def _safe_l2_norm(x: np.ndarray, axis: int = -1, eps: float = 1e-12) -> np.ndarray:
    return np.sqrt((x * x).sum(axis=axis) + eps)


def score_cosine_to_centroids(x: np.ndarray, mu_real: np.ndarray, mu_syn: np.ndarray) -> np.ndarray:
    """Score(x) = cos(x, mu_real) - cos(x, mu_syn). Larger => more likely member."""
    x_n = x / _safe_l2_norm(x, axis=-1, eps=1e-12)[:, None]
    mr = mu_real / (_safe_l2_norm(mu_real, axis=-1, eps=1e-12))
    ms = mu_syn / (_safe_l2_norm(mu_syn, axis=-1, eps=1e-12))
    sim_r = (x_n * mr[None, :]).sum(axis=-1)
    sim_s = (x_n * ms[None, :]).sum(axis=-1)
    return sim_r - sim_s


def score_l2_to_centroids(x: np.ndarray, mu_real: np.ndarray, mu_syn: np.ndarray) -> np.ndarray:
    """Score(x) = ||x - mu_syn||_2 - ||x - mu_real||_2. Larger => more likely member."""
    d_r = _safe_l2_norm(x - mu_real[None, :], axis=-1)
    d_s = _safe_l2_norm(x - mu_syn[None, :], axis=-1)
    return d_s - d_r


def score_wasserstein1_to_centroids(x: np.ndarray, mu_real: np.ndarray, mu_syn: np.ndarray) -> np.ndarray:
    """A light-weight 1-Wasserstein proxy on embedding coordinates.

    Treat the embedding coordinates as equally-weighted 1D samples.
    For equal-length vectors, W1(x, mu) = mean(|sort(x) - sort(mu)|).

    Score(x) = W1(x, mu_syn) - W1(x, mu_real). Larger => more likely member.
    """
    x_sorted = np.sort(x, axis=-1)
    mr_sorted = np.sort(mu_real)
    ms_sorted = np.sort(mu_syn)
    w_r = np.mean(np.abs(x_sorted - mr_sorted[None, :]), axis=-1)
    w_s = np.mean(np.abs(x_sorted - ms_sorted[None, :]), axis=-1)
    return w_s - w_r
...
def run_for_dataset_model(
    ...
    for N in train_sizes:
        ...
        # Distance-based attacker uses class centroids (means) fitted on train split
        mu_real = emb_real_train.mean(axis=0)
        mu_syn = emb_syn_train.mean(axis=0)
        ...
        for noise_label, noise_flag in [("clean", False), ("noise", True)]:
            ...
            # Build labels once (1=REAL/member, 0=SYN/non-member)
            y_true = np.array([1] * len(real_eval) + [0] * len(syn_eval))

            # Three distance-based scores (larger => more likely member)
            methods = [
                ("cosine", score_cosine_to_centroids),
                ("l2", score_l2_to_centroids),
                ("wasserstein1", score_wasserstein1_to_centroids),
            ]

            fig_name = f"{dataset}_{model_short}_{N}_{noise_label}_dist3.png"
            ...
            for method_name, scorer in methods:
                scores_real = scorer(real_eval, mu_real, mu_syn)
                scores_syn = scorer(syn_eval, mu_real, mu_syn)
                ...
                label_txt = f"{method_name}: AUC={auc_val:.3f}, Acc={acc:.3f}, TPR@1%FPR={tpr_1:.3f}"
                plt.plot(fpr, tpr, label=label_txt)

                summary_lines.append(label_txt)
            ...
            plt.title(f"{dataset} – {model_short}, N={N}, {noise_label} (3 distances)")
...
def main():
    ...
    # -------- Summary table --------
    print("\n=== SUMMARY (36 configs) ===")
    header = ["dataset", "model", "train_size", "noise", "method", "accuracy", "auc", "tpr_at_1fpr"]
    print("{:<8} {:<7} {:<10} {:<7} {:<12} {:<10} {:<8} {:<12}".format(*header))

    csv_path = "results_summary.csv"
    with open(csv_path, "w", encoding="utf-8") as f:
        f.write(",".join(header) + "\n")
        for r in results:
            print("{:<8} {:<7} {:<10} {:<7} {:<12} {:<10.4f} {:<8.4f} {:<12.4f}".format(
                r["dataset"], r["model"], r["train_size"], r["noise"], r["method"],
                r["accuracy"], r["auc"], r["tpr_at_1fpr"]
            ))
            f.write("{dataset},{model},{train_size},{noise},{method},{accuracy:.6f},{auc:.6f},{tpr_at_1fpr:.6f}\n".format(**r))
